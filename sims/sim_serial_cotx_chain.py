...
class Simulation:
    ...
    def simulate_pedigree(self, pedigree_dict=None):
        if not pedigree_dict:
            pedigree_dict = {}
            for _, node_id in enumerate(['P1', 'P2']):
                genome = Genome.from_reference()
                genome.genome = genome.genome + _
                genome.id = node_id
                pedigree_dict[node_id] = genome
        
        meiotic_sibling_probabilities = [prob_meiotic_sibling() for _ in range(3)]
        sample_probabilities = np.random.random(3)
        meiotic_sibling_gates = sample_probabilities < meiotic_sibling_probabilities
        

        if meiotic_sibling_gates[0] == True:
            genome_pool = meiosis(pedigree_dict['P1'], pedigree_dict['P2'], N=4)
            pedigree_dict['F11'] = genome_pool[0]
            pedigree_dict['F12'] = genome_pool[1]
        else:
            pedigree_dict['F11'] = meiosis(pedigree_dict['P1'], pedigree_dict['P2'], N=1, v=self.v, bp_per_cM = self.bp_per_cM)[0]
            pedigree_dict['F12'] = meiosis(pedigree_dict['P1'], pedigree_dict['P2'], N=1, v=self.v, bp_per_cM = self.bp_per_cM)[0]
            
        if meiotic_sibling_gates[1] == True:
            genome_pool = meiosis(pedigree_dict['F11'], pedigree_dict['F12'], N=4)
            pedigree_dict['F21'] = genome_pool[0]
            pedigree_dict['F22'] = genome_pool[1]
        else:
            pedigree_dict['F21'] = meiosis(pedigree_dict['F11'], pedigree_dict['F12'], N=1, v=self.v, bp_per_cM = self.bp_per_cM)[0]
            pedigree_dict['F22'] = meiosis(pedigree_dict['F11'], pedigree_dict['F12'], N=1, v=self.v, bp_per_cM = self.bp_per_cM)[0]
        
        if meiotic_sibling_gates[2] == True:
            genome_pool = meiosis(pedigree_dict['F21'], pedigree_dict['F22'], N=4)
            pedigree_dict['F31'] = genome_pool[0]
            pedigree_dict['F32'] = genome_pool[1]
        else:
            pedigree_dict['F31'] = meiosis(pedigree_dict['F21'], pedigree_dict['F22'], N=1, v=self.v, bp_per_cM = self.bp_per_cM)[0]
            pedigree_dict['F32'] = meiosis(pedigree_dict['F21'], pedigree_dict['F22'], N=1, v=self.v, bp_per_cM = self.bp_per_cM)[0]
...
if __name__ == '__main__':
    ...
    file_basename = '/Users/weswong/Documents/GitHUb/Pedigree/sims/serial_cotx_chain/serial_cotx_chain_{i}'.format(i=iteration)
